...
theta = linspace(0, 1, size(oxdata, 1))';
t_garb = 1.06*theta + 1.045*(1.0 - theta);
u_garb = 1.0*theta + 1.045*(1.0 - theta);
oxdata(:,9)  = u_garb.*oxdata(:,9);
oxdata(:,12) = t_garb.*oxdata(:,12);

inner_Scale(oxdata, 9,  'purdue/cs-u.csv', 'U tilde', 'png/utild.png', 1);
inner_Scale(oxdata, 12, 'purdue/cs-T.csv', 'T tilde', 'png/Ttild.png', 2);
